...
public partial class InfoBarPanel : Panel
{
    protected override Size MeasureOverride(Size availableSize)
    {
        ...
        if (
            nItems == 1
            || totalWid > availableSize.Width
            || (minHeight > 0 && hgtOfTallestInHorizontal > minHeight)
        )
        {
            _isVertical = true;
            var vertPad = VerticalOrientationPadding;

            return new Size(widOfWidest + vertPad.Horizontal(), totalHgt + vertPad.Vertical());
        }

        _isVertical = false;
        var horPad = HorizontalOrientationPadding;

        return new Size(totalWid + horPad.Horizontal(), hgtOfTallest + horPad.Vertical());
    }
                        ...
                        Children[i]
                            .Arrange(
                                new Rect(
                                    horOff,
                                    horPad.Top + horMarg.Top,
                                    Math.Max(desSize.Width, finalSize.Width - horOff),
                                    desSize.Height
                                )
                            );
                    }
