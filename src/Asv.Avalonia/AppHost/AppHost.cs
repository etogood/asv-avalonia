...
public sealed class AppHost:IAppHost
{
    ...
    public static IAppHost Instance
    {
        get
        {
            if (_instance == null) throw new InvalidOperationException($"{nameof(AppHost)} not initialized. Please call {nameof(Initialize)} method first.");
            return _instance;
        }
    }
    ...
    {
        if (_instance != null) throw new InvalidOperationException($"{nameof(AppHost)} already initialized.");
    ...
    internal AppHost(IConfiguration config,
        AppPath appPath,
        AppInfo appInfo,
        ILoggerFactory logFactory, AppArgs args, 
        string? mutexName,
        string? argsPipeName)
    {
        ...
        if (argsPipeName != null)
        {
            if (_mutex == null)
            {
                ...
                throw new InvalidOperationException("Named pipe can be used only with single instance mode.");
            }
            if (IsFirstInstance)
            {
        ...
        new Thread(start: () =>
            {
                while (true)
                {
                    try
                    {
                        ...
                        if (args != null)
                        {
                            logger.ZLogInformation($"Received arguments from the named pipe {pipeName}.");
    ...
    private void SendArgumentsToRunningInstance(AppArgs args, string pipeName, ILogger logger)
    {
        try
        {
            logger.ZLogInformation($"Sending arguments to the running instance through the named pipe {pipeName}.");
        ...
        {
            logger.ZLogError(ex,$"Failed to send arguments to the running instance through the named pipe {pipeName}.");
        }
    }
