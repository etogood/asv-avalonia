...
public class PaginationCommand : ContextCommand<ISupportPagination, ListArg>
{
    ...
    public override ValueTask<ListArg?> InternalExecute(ISupportPagination context, ListArg arg, CancellationToken cancel)
    {
        var old = new ListArg(2)
        {
            new IntArg(context.Skip.Value),
            new IntArg(context.Take.Value),
        };
    ...
    {
        return context.ExecuteCommand(Id, new ListArg(2)
        {
            new IntArg(skip),
            new IntArg(take),
        });
